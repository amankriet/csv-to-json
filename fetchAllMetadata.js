...
for (let i = 1; i < array.length - 1; i++) {
    ...
    for (let j in headers) {
        if (headers[j] == "metadata") {
            if (properties[j].includes(", ")) {
                productMetadata = properties[j]
                    .split(", ").map(item => item.trim()).join(",")
            } else if (properties.includes(",")) {
                productMetadata = properties[j]
                    .split(",").map(item => item.trim()).join(",")
            }
            else productMetadata = properties[j];
        }
    }

    // Add the product title in the array.
    result.push(productMetadata)
}

// Convert the resultant array to json and
// generate the JSON output file.
let json = JSON.stringify(result);
fs.writeFileSync('output_product_metadata.json', json);
