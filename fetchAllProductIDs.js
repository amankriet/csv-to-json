...
// fs npm package
const fs = require("fs");
csv = fs.readFileSync("PRODUCTS_DATA.csv")
...
for (let i = 1; i < array.length - 1; i++) {
    ...
    for (let j in headers) {
        // console.log(j, headers[j], properties[j])
        if (headers[j] == "productID") {
            productID = properties[j];
        }
...
fs.writeFileSync('output_product_ids.json', json);
